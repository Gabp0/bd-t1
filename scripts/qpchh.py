...
def calculate_qpchh(query_times:np.ndarray, refresh_times:np.ndarray, total_time:float, scale_factor:int) -> float:
    """
    ...
    ----------
    query_times : np.ndarray
        The completion times of each query during the power test
    ...
    throughput_size = (QUERY_NUMBER * 22 * 3600) / (total_time * scale_factor) # TPC-H 5.4.2.1
...
if __name__ == "__main__":
    if len(argv) < 3:
        print("Usage: qpchh <power test log> <throughput test log> [scale factor]")
        exit(1)
    ...
    qpchh = calculate_qpchh(query_times, refresh_times, total_time, scale_factor)
